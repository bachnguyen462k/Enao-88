...
export default function Matches() {
  const matches = [
    {
      league: 'Premier League • Gameweek 9',
      status: 'Live • 64\'',
      isLive: true,
      tv: 'TV: SKY SPORTS',
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 px-4 py-2 bg-surface-container text-on-surface rounded-lg border border-white/5 hover:bg-surface-container-highest transition-all">
          ...
          <div className="flex bg-surface-container-low p-1 rounded-xl gap-1">
            <button className="px-4 py-1.5 text-xs font-bold bg-[#00E676] text-[#00210b] rounded-lg shadow-sm">All</button>
            <button className="px-4 py-1.5 text-xs font-bold text-slate-400 hover:text-on-surface transition-colors">Live</button>
            <button className="px-4 py-1.5 text-xs font-bold text-slate-400 hover:text-on-surface transition-colors">Upcoming</button>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <select className="bg-surface-container text-on-surface border-none text-xs font-bold rounded-lg py-2 px-4 focus:ring-1 focus:ring-[#00E676]/30 cursor-pointer appearance-none pr-10 relative">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        {matches.map((match, idx) => (
          <div key={idx} className="bg-surface-container rounded-xl p-6 relative overflow-hidden group border border-white/5 hover:border-[#00E676]/20 transition-all duration-300">
            <div className="flex justify-between items-start mb-6">
              <div className="flex flex-col gap-1">
                <span className="text-[10px] font-bold tracking-widest text-[#00E676] uppercase">{match.league}</span>
                <div className="flex items-center gap-2">
                  {match.isLive && <span className="w-2 h-2 rounded-full bg-[#00E676] animate-pulse"></span>}
                  <span className={`text-xs font-bold uppercase tracking-tighter ${match.isLive ? 'text-white' : 'text-slate-400'}`}>{match.status}</span>
                </div>
              </div>
              <div className="bg-surface-container-highest px-3 py-1 rounded-sm text-[10px] font-bold text-slate-300">{match.tv}</div>
            </div>
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex flex-col items-center justify-center px-4">
                <div className={`font-headline font-black tracking-tighter ${match.score === 'VS' ? 'text-xl text-slate-500 uppercase' : 'text-4xl text-white'}`}>{match.score}</div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-3 gap-2">
                <div className="bg-surface-container-low p-3 rounded-xl border border-white/5 flex flex-col items-center">
                  <span className="text-[10px] font-bold text-slate-500 mb-1">HOME</span>
                  <span className={`text-sm font-headline font-black ${idx === 0 ? 'text-[#00E676]' : 'text-slate-300'}`}>{match.odds.home}</span>
                </div>
                <div className="bg-surface-container-low p-3 rounded-xl border border-white/5 flex flex-col items-center">
                  <span className="text-[10px] font-bold text-slate-500 mb-1">DRAW</span>
                  <span className="text-sm font-headline font-black text-slate-300">{match.odds.draw}</span>
                </div>
                <div className="bg-surface-container-low p-3 rounded-xl border border-white/5 flex flex-col items-center">
                  <span className="text-[10px] font-bold text-slate-500 mb-1">AWAY</span>
                  <span className="text-sm font-headline font-black text-slate-300">{match.odds.away}</span>
                </div>
              </div>
              <div className="space-y-2">
                <div className="flex justify-between text-[10px] font-bold text-slate-400">
                  <span>AI WIN PROBABILITY</span>
                  <span className="text-[#00E676]">{match.accuracy} ACCURACY</span>
                </div>
                <div className="h-1.5 w-full bg-outline-variant/30 rounded-full flex overflow-hidden">
                  <div className="h-full bg-primary shadow-[0_0_8px_rgba(117,255,158,0.4)]" style={{ width: `${match.probabilities.home}%` }}></div>
                  <div className="h-full bg-slate-600" style={{ width: `${match.probabilities.draw}%` }}></div>
                  <div className="h-full bg-slate-800" style={{ width: `${match.probabilities.away}%` }}></div>
                </div>
                <div className="flex justify-between text-[9px] font-bold text-slate-500 mt-1">
                  <span>{match.probabilities.home}% Home</span>
                  <span>{match.probabilities.draw}% Draw</span>
                  <span>{match.probabilities.away}% Away</span>
                </div>
              </div>
              <button className="w-full py-3.5 bg-surface-container-highest hover:bg-surface-bright text-on-surface font-headline font-bold text-sm rounded-xl transition-all flex items-center justify-center gap-2 group/btn border border-white/5">
                Predict Score
      ...
      <div className="mt-12 flex justify-center">
        <button className="px-8 py-3 bg-surface-container border border-white/10 text-on-surface font-headline font-bold text-sm rounded-full hover:bg-surface-container-highest transition-all">
          Load More Matches
        </button>
      </div>
    </div>
